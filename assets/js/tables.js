    ...
    var initDetailedViewTable = function() {
        ...
        var _format = function(d) {
            ...
            return '<table class="table table-inline">' +
                '<tr>' +
                '<td>Visualizar lista de atividades: status <span class="label label-important">ALERTA!</span></td>' +
                '<td></td>' +
                '</tr>' +
                '<tr>' +
                '<td>Evidências cadastradas no período: 245</td>' +
                '<td></td>' +
                '</tr>' +
        ...
        $('#detailedTable tbody').on('click', 'tr td:first-child', function() {
            var row = $(this).parent()
            if (row.hasClass('shown') && row.next().hasClass('row-details')) {
